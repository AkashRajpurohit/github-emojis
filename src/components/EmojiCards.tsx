...
const emojiReducer = (state: State, action: Action): State => {
	...
	switch (type) {
		case 'LOAD_MORE': {
			const newLength = Math.max(40, Math.min(emojisLength + 40, filteredEmojis.length));
    ...
    case 'SEARCH': {
      ...
      const searchResults = totalEmojis.filter(({ code }) => code.toLowerCase().includes(searchTerm));
...
export const EmojiCards = ({ emojis }: IEmojiCardProps) => {
  ...
  const filterOnSearch = useCallback((value: string) => {
	...
	return (
		<Stack pt='8' w='70%'>
			<Box mb='4'>
				<Input placeholder='Search Shortcodes' onChange={(e) => {
          filterOnSearch(e.target.value);
			...
			>
				...
				{
					state.emojis.length === 0 && <Text my="8">No Emoji Found for this search</Text>
				}
